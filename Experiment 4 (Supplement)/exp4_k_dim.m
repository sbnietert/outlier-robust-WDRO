...
d_y = 3; % label dimension
sample_sizes = [10, 20, 50, 75, 100];%, 200, 500];
T = 10; % iterations
rho = 0.1; % Wp perturbation size (here, will be a translation)
eps = 0.1; % TV perturbation size
sigma = sqrt(d + d_y); % moment bound (sqrt(dim) * covariance)
contamination_scale_factor = 20; % push to make standard WDRO have unbounded excess risk
...
for t = 1:T
    ...
    for i = 1:length(sample_sizes)
        ...
        [theta_star, obj_star] = regular_WDRO_k_dim(X_tilde(1:m,:), y_tilde(1:m,:), rho, dual_norm, verbose);
        standard_excess_risk = mean(sum(abs(y - X*theta_star))) - best_risk;
        ...
        z_0(end+1:end+d_y) = cheap_robust_mean_estimate(y_tilde, 2*eps);

        [theta_star2, obj_star2] = outlier_robust_WDRO_k_dim(X_tilde(1:m,:), y_tilde(1:m,:), sigma, rho, 0, z_0', dual_norm, verbose);
        outlier_robust_excess_risk = mean(sum(abs(y - X*theta_star2))) - best_risk;
...
%% bootstrapping
outlier_robust_0_excess_risks = outlier_robust_0_excess_risks(1:10,:);
standard_excess_risks = standard_excess_risks(1:10,:);
...
xlabel("# samples")
xlim([10,100])
ylabel("excess risk (ℓ₁ mean absolute deviation)")
